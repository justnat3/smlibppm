...
class PPM(object):
    ...
    def __init__(self, width: int, height: int):
        ...
        if not isinstance(width, int):
            raise ValueError(f"width Expected int but found -> {type(width)}")

        # break code if not the right type
        if not isinstance(height, int):
            raise ValueError(f"height Expected int but found -> {type(height)}")
    ...
    def init_buffer(self, width: int, height: int):
        """[enabling people to create multiple pixel maps, this will not keep state of the old print map however]
        ...
        self.width = width 
        self.height = height
        self.pixels = [["255", " 255", " 255 "] for i in range(0, self.width * self.height)]
    ...
    def fill_rect(self, color: list, wb: int, hb: int, roi: list):
        ...
        if not isinstance(wb, int):
            raise ValueError(f"widthbox Expected int but found -> {type(hb)}")

        if not isinstance(hb, int):
            raise ValueError(f"heightbox Expected int but found -> {type(wb)}")

        if not isinstance(roi, list):
            raise ValueError(f"roi Expected list but found -> {type(wb)}")

        if len(roi) > 2 or len(roi) < 1:
            raise ValueError(f"invalid length: roi- Expected 2 but found -> {len(roi)}")
    ...
    def put_pixel(self, color: list, x: int, y: int) -> bool:
        ...
        if not isinstance(color, list):
            raise ValueError(f"color: Expected list but found -> {len(roi)}")
    ...
    def draw_line_rand_colors(start: tuple, end: tuple) -> bool:
        """
            Draws line of a specified color in a specified position
